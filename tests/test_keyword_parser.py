...
def test_config_ini_is_read_correctly():
    ...
    e.g.
        [ats1]
        intel-18.0.5-mpich-7.7.6:   << kp.config["ats1"].keys()
            intel-18             <-|
            intel                  |-< kp.config["ats1"].values()
            default-env          <-|
        intel-19.0.4-mpich-7.7.6:   << kp.config["ats1"].keys()
            intel-19                << kp.config["ats1"].values()
    """
    assert [_ for _ in kp.config.keys()] != ["DEFAULT"]
    assert [_ for _ in kp.config["ats1"].keys()] != []
    assert "default-env-hsw" in kp.get_values_for_section_key(
        "ats1", "intel-19.0.4-mpich-7.7.15-hsw-openmp"
    )
...
def test_values_do_not_contain_whitespace_or_delimiter(multiple_values):
    ...
    with pytest.raises(SystemExit) as excinfo:
        ...
        for key in kp.config["ats1"].keys():
            kp.get_values_for_section_key("ats1", key)
...
def test_values_are_unique():
    ...
    assert ("Values for 'bad_config.ini'['ats1']['intel-18.0.5-mpich-7.7.15'] "
...
def test_values_do_not_match_any_keys(bad_value):
    ...
    assert "Value found for 'ats1' that matches a key:" in exc_msg
...
def test_general_value_matches_correct_key(general_section_order):
    ...
    kp = KeywordParser(filename)
    matched_key = kp.get_key_for_section_value("ats1", "intel")
...
def test_matched_value_not_in_config_section_raises():
    ...
    with pytest.raises(SystemExit) as excinfo:
        kp.get_key_for_section_value("ats1", "bad_value")
    exc_msg = excinfo.value.args[0]
